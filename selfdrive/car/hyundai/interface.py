...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, disable_openpilot_long, experimental_long, docs):
    ...
    if params.get_bool("HKGtuning"):
        ...
        ret.longitudinalTuning.kpBP = [0.0, 5.0, 10.0, 20.0]  # Speed breakpoints in m/s
        ret.longitudinalTuning.kiBP = [0.0, 5.0, 10.0, 20.0]
        ret.longitudinalTuning.kiV = [0.0, 0.01, 0.02, 0.02]
        ...
        if is_canfd_car:
            ret.longitudinalTuning.kpV = [0.45, 0.50, 0.55, 0.50]
            ret.stoppingDecelRate = 0.3
        else:
            ret.longitudinalTuning.kpV = [0.30, 0.35, 0.40, 0.50]
    ...
    unsupported_long_cars = (
        ...
        if is_canfd_car
        else UNSUPPORTED_LONGITUDINAL_CAR | CAMERA_SCC_CAR
    )
  ...
  #Initialize radar tracks if enabled in FrogPilot flags.
  @staticmethod
