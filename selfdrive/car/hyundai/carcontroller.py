...
class CarController(CarControllerBase):
  ...
  def cal_jerk(self, accel, actuators):
    """Calculate jerk based on acceleration changes"""
    if actuators.longControlState == LongCtrlState.off:
        accel_diff = 0.0
    else:
        accel_diff = accel - self.accel_last_jerk

    accel_diff /= DT_CTRL
    self.jerk = self.jerk * 0.9 + accel_diff * 0.1
    self.accel_last_jerk = accel  # Store for next iteration
    return self.jerk
  ...
  def update(self, CC, CS, now_nanos, frogpilot_toggles):
    ...
    if frogpilot_toggles.sport_plus:
      accel = clip(actuators.accel, CarControllerParams.ACCEL_MIN, min(frogpilot_toggles.max_desired_acceleration, get_max_allowed_accel(CS.out.vEgo)))
    elif self.hkg_tuning and self.CP.openpilotLongitudinalControl:
       ...
       jerk_max = interp(self.jerk_count, [0, 1.5, 2.5],
                        [startingJerk, startingJerk, jerkLimit])
       ...
       if actuators.longControlState == LongCtrlState.off:
          ...
          self.jerk_lower_limit = min(max(1.0, -jerk * 3.0), jerkLimit)

        # Calculate acceleration with jerk limits
       accel_delta = accel - self.accel_last
       limited_accel_delta = clip(accel_delta,
                                -self.jerk_lower_limit * DT_CTRL,
                                self.jerk_upper_limit * DT_CTRL)
